...
function isDialogPortaledLayer(target: EventTarget | null) {
  ...
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ...
      onPointerDownOutside={(e) => {
        if (isDialogPortaledLayer(e.target)) e.preventDefault();
        onPointerDownOutside?.(e);
      }}
      onFocusOutside={(e) => {
        if (isDialogPortaledLayer(e.target)) e.preventDefault();
      ...
      className={cn(
        "fixed left-[50%] top-[50%] z-[100] grid w-[calc(100%-2rem)] max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 border bg-background p-6 shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 sm:rounded-lg max-h-[min(90vh,800px)] overflow-y-auto",
        className
      )}
      {...props}
    >
